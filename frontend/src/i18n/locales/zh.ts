  ...
  admin: {
    ...
    settings: {
      ...
      payment: {
        ...
        field_paymentMethod: '支付方式',
        field_locale: '语言/地区',
        field_emailTo: '收款/接收邮箱',
        field_mintTo: '接收钱包地址',
        field_extraPayload: '额外请求 JSON',
        ...
        field_crossmintApiBaseHint: '测试密钥使用 https://staging.crossmint.com/api，生产密钥使用 https://www.crossmint.com/api。',
        field_crossmintPaymentMethodHint: '默认 fiat。可按 Crossmint Checkout 配置填写 fiat、crypto 或 fiat,crypto 等受支持值。',
        field_crossmintCurrencyHint: '默认 USD。请选择该 Crossmint listing 配置支持的结算/收银台币种。',
        field_crossmintExtraPayloadHint: '可选 JSON 对象，会合并到 Crossmint hosted checkout link 请求中，用于高级配置。',
        ...
        crossmintWebhookHint: '请在 Crossmint 后台将以下地址配置为 Checkout V2 Webhook 端点，并使用该端点生成的签名密钥。事件选择 purchase.succeeded / purchase.failed；当前 hosted-link 集成不支持 Checkout V3 orders.* 事件。',
        crossmintGuideSummary: '创建具备 Payments 权限的 Crossmint server API key，并配置支持法币和数字货币收银台的 listing/client ID。',
        crossmintGuideNote: '首版实现使用 hosted checkout link 和 Checkout V2 passthrough metadata。Sub2API 会把订单号写入 whPassThroughArgs，用于支付成功/失败事件回填订单。',
        limitsTitle: '限额配置',
        limitSingleMin: '单笔最低',
        limitSingleMax: '单笔最高',
        limitDaily: '每日限额',
